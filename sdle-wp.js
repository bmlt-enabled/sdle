...
jQuery('#data-layers-popdensity-enabled').click(function() {
    clearKmlLayers();
    if (jQuery('#data-layers-popdensity-enabled').is(":checked")) {
...
function getServiceBodyForCoordinates(latitude, longitude, callback) {
    jQuery.getJSON(root + "/client_interface/jsonp/?switcher=GetSearchResults&sort_results_by_distance=1&geo_width=-10&long_val=" + longitude + "&lat_val=" + latitude + "&callback=?", function (data) {
        callback(data);
    });
...
function getMeetingsForServiceBody(id, recurse, callback) {
    ...
    jQuery.getJSON(root + url + "&callback=?", function (data) {
        callback(data);
    });
}

function search() {
    geocoder.geocode({'address': jQuery("#criteria").val() }, function(results, status) {
        if (status === "OK") {
            setMapInfo(pos = {
        ...
        if (drawOption === "polygon") {
            ...
            for (var j = 0; j < data.length; j++) {
                ...
                var circle = new google.maps.Circle({
                    map: map,
                    radius: parseFloat(jQuery("#willingness").val()) * radius_to_miles_ratio,
                    fillColor: 'blue',
                    strokeWeight: 0.5,
                    fillOpacity: 0.05,
                });
